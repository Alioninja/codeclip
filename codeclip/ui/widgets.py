...
class ActionButton(Button):
    """Button for actions section with up/down/left/right navigation."""
    
    # Navigation layout:
    # Row 0: btn-generate (full width)
    # Row 1: btn-select-all, btn-deselect-all (half width each)
    # Row 2: btn-change-dir (full width)
    ...
    def _find_position(self):
        """Find current button's row and column in layout."""
        for row_idx, row in enumerate(self.BUTTON_LAYOUT):
            if self.id in row:
                col_idx = row.index(self.id)
                return row_idx, col_idx
        return -1, -1
    
    def _focus_button(self, button_id: str):
        """Focus a button by id."""
        try:
            self.app.query_one(f"#{button_id}").focus()
        except Exception:
            pass
    
    def action_nav_up(self):
        """Navigate up to previous row."""
        row, col = self._find_position()
        if row > 0:
            target_row = self.BUTTON_LAYOUT[row - 1]
            # Pick first button in target row
            self._focus_button(target_row[0])
    
    def action_nav_down(self):
        """Navigate down to next row."""
        row, col = self._find_position()
        if row < len(self.BUTTON_LAYOUT) - 1:
            target_row = self.BUTTON_LAYOUT[row + 1]
            # Pick first button in target row
            self._focus_button(target_row[0])
    ...
    def action_nav_left(self):
        ...
        row, col = self._find_position()
        if row >= 0:
            current_row = self.BUTTON_LAYOUT[row]
            if col > 0:
                self._focus_button(current_row[col - 1])
