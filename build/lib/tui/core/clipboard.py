...
def copy_to_clipboard(text: str) -> tuple[bool, str]:
    ...
    try:
        if system == "Linux":
            ...
            # Use clip.exe with UTF-16LE encoding for proper Unicode support
            # This properly handles box-drawing characters (└, ├, │)
            process = subprocess.Popen(
                ['clip'],
                stdin=subprocess.PIPE, stderr=subprocess.PIPE
            )
            process.communicate(text.encode('utf-16-le'), timeout=2)
            if process.returncode == 0:
                return True, "✓ Copied (clip)"
    except Exception:
        pass
